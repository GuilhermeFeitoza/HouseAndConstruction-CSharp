...
namespace BLL
{
    public class Lancamento
    {
        ...
        public void Incluir()
        {
            try
            {
                SqlParameter[] listaComParametros = {
                    ...
                    new SqlParameter("@DataPagamento", SqlDbType.DateTime) {Value=_DataPagamento},
                    new SqlParameter("@statuspagtitulo", SqlDbType.Bit) {Value=_StatusPagTitulo},
    
                };
                instrucaoSql = "INSERT INTO tbLancamento (CodigoTitulo, DataVencimento, ValorTitulo, StatusPagTitulo, CodigoUsuario,DataPagamento) VALUES (@codtit, @datavenc, @valortitulo, @statuspagtitulo, @codusu,@DataPagamento)";
        ...
        public double ValorContas(byte tipostatus, DateTime datainicial, DateTime datafinal)
        {
            try
            {
                instrucaoSql = "SELECT sum(ValorTitulo)as ValorTitulo from tblancamento WHERE StatusPagTitulo=" + tipostatus + " AND (DataVencimento BETWEEN '" + datainicial + "' and '" + datafinal + "')";
                SqlDataReader ddr;
                ddr=c.RetornarDataReader(instrucaoSql);
                ddr.Read();
                if (ddr["ValorTitulo"] == DBNull.Value)
                {
                    return 0;
                }
                             
                    return Convert.ToDouble(ddr[0]);
