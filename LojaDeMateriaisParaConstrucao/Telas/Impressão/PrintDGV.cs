...
namespace LojaDeMateriaisParaConstrucao.Telas.Impressão
{
    ...
    public class printDGV
    {
        ...
        private static System.Drawing.Printing.PrintDocument printDoc =
                       new System.Drawing.Printing.PrintDocument();  // Tipo de objeto de documento de impressão para imprimir

        private static string PrintTitle = "";  // Cabeçalho das páginas
        private static DataGridView dgv;        // "Guarda" o objeto referente ao conteúdo da DataGridView
        ...
        public static void Print_DataGridView(DataGridView dgv1)
        {
            ...
            try
            {
                // Obtém objeto para imprimir do DataGridView
                dgv = dgv1;
                    ...
                    System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            try
            {
                ...
                if (PageNo == 1)
                {
                    ...
                    {
                        if (!GridCol.Visible) continue;
                        // Ignora a coluna que não está selecionada
                        if (!printDGV.SelectedColumns.Contains(GridCol.HeaderText)) continue;
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Desenha o cabeçalho
                            Bitmap bmp = Properties.Resources._57c7cc92_1a58_4ea3_8826_58bfb617fec7;
                            Image newImage = bmp;
                    ...
                    int RowsPerPage)
        {
            ...
            //string PageNum = PageNo.ToString() + " deeee " + Math.Ceiling((double)(cnt / RowsPerPage)).ToString();

            string PageNum;
            if (Math.Ceiling((double)(cnt / RowsPerPage)) == 0)
            {
                PageNum = PageNo.ToString();
            }
            else
            {
                PageNum = PageNo.ToString() + " de " + Math.Ceiling((double)(cnt / RowsPerPage)).ToString();
            }
